...
```
 ...
 schema ──► preload ┤                 ├──► tablespace-report
                    └─ MYSQL_HOST_V7 ─┘            │
                                                   ▼
                              if data+index ≳ 50 GiB → flag AWS SA
                                                   │
...
4. **tablespace-report** — after preload, before measure. If `DATA_LENGTH + INDEX_LENGTH` ≳ **50 GiB**, stop and revisit storage with the AWS SA.
