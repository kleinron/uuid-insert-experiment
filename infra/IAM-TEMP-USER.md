...
Replace `ACCOUNT_ID` and `REGION` (stack default `us-east-1`). Resource names assume `name_prefix` default `payments-exp`. If you change the prefix, replace that string throughout the policy.
...
| --- | --- |
| Existing VPC + `subnet_ids` (this module never creates a VPC) | 2× RDS (`payments-exp-v4`, `payments-exp-v7`) |
| Optional EC2 key pair (`key_name`) | 1× EC2 loadgen (`payments-exp-loadgen`) |
| RDS service-linked role `AWSServiceRoleForRDS` if the account has never used RDS | 3 security groups (`-sg-ec2`, `-sg-rds`, `-sg-sm-endpoint`) + rules |
...
5. On the operator machine that will hold local `*.tfstate`, export `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` (and `AWS_REGION`) and run `make apply` / later `make destroy` as in [README.md](README.md).
...
- **EC2 `Resource: "*"`** on the mutate statement is **intentional**. Security groups, interface VPC endpoints, ENIs, `CreateTags`, and `RunInstances` / `TerminateInstances` do not usefully constrain to a `payments-exp-*` name prefix. The region condition is the bound.
